function latex_multipage_subfigure(file_ltx, str_subfig_names, str_subfig_captions, str_main_caption, str_subfig_labels, str_main_label, varargin)
%
% |----------------------------------------------------------------
% | (C) 2023 Mikus Grasis
% |
...
% |
% |     Date authored: 22.01.2023
% |     Modifications:
% |     22.01.2023 - initial version (MG)
% |----------------------------------------------------------------
%
% Inputs:
...
addParameter(p, 'num_subfigs_per_row', 2, @isscalar);
parse(p, varargin{:});

placement = p.Results.placement;
subfig_width = p.Results.subfig_width;
num_subfigs_per_row = p.Results.num_subfigs_per_row;
...
for curr_subfig = 1:num_subfigs
    ...
    fprintf(file_ltx, '\\caption{%s}\n', str_subfig_captions{curr_subfig});     % \caption{str_subfig_caption}
    fprintf(file_ltx, '\\label{%s}\n', str_subfig_labels{curr_subfig});         % \label{fig_str_curr_label}
    fprintf(file_ltx, '\\end{subfigure}\n');                                    % \end{subfigure}
    fprintf(file_ltx, '%%\n');                                                  % %
    if curr_subfig < num_subfigs && eq(mod(curr_subfig, num_subfigs_per_row), 0)
        fprintf(file_ltx, '\\end{figure}%%\n');                                 % \end{figure}%
        fprintf(file_ltx, '%s\n', str_delim);                                   % %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        fprintf(file_ltx, '\\begin{figure}[%s]\\ContinuedFloat\n', placement);  % \begin{figure}[!h]\ContinuedFloat
        fprintf(file_ltx, '\\centering\n');                                     % \centering
        fprintf(file_ltx, '%%\n');                                              % %
    end
end
